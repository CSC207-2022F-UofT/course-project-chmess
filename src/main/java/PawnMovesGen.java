...
//        Move move1Rel = new Move(pos, new int[]{pos[0], pos[1] + 1});
        ...
        if (pos[0] == 6 && !(isOccupiedByPiece(move1))) { // checks if pawn can promote
            ...
            if (isOccupiedByEnemy(moveCapPro1)) {
                addPromoteMoves(pos, new int[]{pos[0] - 1, 7}, pawn, board);
            }
            if (isOccupiedByEnemy(moveCapPro2)) {
                addPromoteMoves(pos, new int[]{pos[0] + 1, 7}, pawn, board);
            }
        }
        if (pos[0] == 1 && !(this.isOccupiedByPiece(move1))) { // checks if pawn is in starting position and not jumping over piece
        ...
        if (!(isOccupiedByPiece(move1))) { // move one tile
        ...
        if (isOccupiedByEnemy(destCap1)) { // capture
            this.moves.add(destCap1);
        }
        if (isOccupiedByEnemy(destCap2)) {
     ...
     */
    private boolean isOccupiedByPiece (Move move) {
        return isOccupiedByFriendly(move) && isOccupiedByEnemy(move);
    }
    /**
    ...
    private void addPromoteMoves (int[] pos, int[] dest, Piece pawn, Board board) {
        ...
        for (String p : pieces) { // creates move instance for each promotion option
            ...
            Piece piecePromote = pf.create(p);
            int[] destAbs = board.switchCoords(dest);
            piecePromote.setCoords(destAbs[0], destAbs[1]);
            piecePromote.setColor(pawn.getColor());
            PromoteMove promote = new PromoteMove(board.switchCoords(pos), destAbs, piecePromote);
